...
! function(a) {
	...
	a.extend(a.FE.POPUP_TEMPLATES, {
		...
		function m(c, d) {
			var e, f, g = '<table class="table table-bordered" style="width: 100%;"><tbody>',
				h = 100 / d;
			for (e = 0; c > e; e++) {
		...
		function M(c) {
			var d = K(c);
			if (ga().length > 0 && !d && L(), 1 == c.which) {
				if (la = !0, d) {
					ga().length > 0 && !c.shiftKey && L(), c.stopPropagation(), b.events.trigger("image.hideResizer"), b.events.trigger("video.hideResizer"), ka = !0;
					var e = d.tagName.toLowerCase();
					c.shiftKey && b.$el.find(e + ".fr-selected-cell").length > 0 ? a(b.$el.find(e + ".fr-selected-cell").closest("table")).is(a(d).closest("table")) ? J(ma, d) : F() : ((b.keys.ctrlKey(c) || c.shiftKey) && F(), ma = d, J(ma, ma))
				}
			} else 3 == c.which && d && L()
		}
		...
		function N(c) {
			if (1 == c.which) {
				...
				if (oa) {
					oa = !1, ia.removeClass("fr-moving"), b.$el.removeClass("fr-no-selection"), b.edit.on();
					var f = parseFloat(ia.css("left")) + b.opts.tableResizerOffset;
					b.opts.iframe && (f -= b.$iframe.offset().left), ia.data("release-position", f), ia.removeData("max-left"), ia.removeData("max-right"), Z(c), R()
				}
			}
		}
		...
		function O(c) {
			if (ka === !0) {
				...
				if (d.closest("table").is(ha())) {
					if ("TD" == c.currentTarget.tagName && 0 === b.$el.find("th.fr-selected-cell").length) return void J(ma, c.currentTarget);
					if ("TH" == c.currentTarget.tagName && 0 === b.$el.find("td.fr-selected-cell").length) return void J(ma, c.currentTarget)
				}
				F()
			}
		}
		...
		function Q() {
			b.shared.$table_resizer || (b.shared.$table_resizer = a('<div class="fr-table-resizer"><div></div></div>')), ia = b.shared.$table_resizer, b.events.$on(ia, "mousedown", function(a) {
				return b.core.sameInstance(ia) ? (ga().length > 0 && L(), 1 == a.which ? (oa = !0, ia.addClass("fr-moving"), F(), ia.find("div").css("opacity", 1), !1) : void 0) : !0
			}), b.events.$on(ia, "mousemove", function(a) {
				return b.core.sameInstance(ia) ? void(oa && (b.opts.iframe && (a.pageX -= b.$iframe.offset().left), aa(a))) : !0
			}), b.events.on("shared.destroy", function() {
				ia.html("").removeData().remove()
			}, !0), b.events.on("destroy", function() {
				b.$el.find(".fr-selected-cell").removeClass("fr-selected-cell"), ia.hide().appendTo(a("body"))
			}, !0)
		}

		function R() {
			ia && (ia.find("div").css("opacity", 0), ia.css("top", 0), ia.css("left", 0), ia.css("height", 0), ia.find("div").css("height", 0), ia.hide())
		}

		function S() {
			ja && ja.removeClass("fr-visible").css("left", "-9999px")
		}

		function T(c, d) {
			var e = a(d),
				f = e.closest("table");
			if (d && "TD" != d.tagName && "TH" != d.tagName && (e.closest("td").length > 0 ? d = e.closest("td") : e.closest("th").length > 0 && (d = e.closest("th"))), !d || "TD" != d.tagName && "TH" != d.tagName) ia && e.get(0) != ia.get(0) && e.parent().get(0) != ia.get(0) && b.core.sameInstance(ia) && R();
			else {
				...
				if (Math.abs(c.pageX - g) <= b.opts.tableResizerOffset || Math.abs(h - c.pageX) <= b.opts.tableResizerOffset) {
					...
					ia || Q(), ia.data("table", f), ia.data("first", i), ia.data("second", j), ia.data("instance", b), b.$wp.append(ia);
					var v = k - b.win.pageXOffset - b.opts.tableResizerOffset,
						w = q - b.win.pageYOffset;
					b.opts.iframe && (v += b.$iframe.offset().left - a(b.o_win).scrollLeft(), w += b.$iframe.offset().top - a(b.o_win).scrollTop(), l += b.$iframe.offset().left, m += b.$iframe.offset().left), ia.data("max-left", l), ia.data("max-right", m), ia.data("origin", k - b.win.pageXOffset), ia.css("top", w), ia.css("left", v), ia.css("height", r), ia.find("div").css("height", r), ia.css("padding-left", b.opts.tableResizerOffset), ia.css("padding-right", b.opts.tableResizerOffset), ia.show()
				} else b.core.sameInstance(ia) && R()
			}
		}

		function U(c, d) {
			if (b.$box.find(".fr-line-breaker").is(":visible")) return !1;
			ja || da(), b.$box.append(ja), ja.data("instance", b);
			var e = a(d),
				f = e.find("tr:first"),
				g = c.pageX,
				h = 0,
		...
		function Y() {
			if (oa) {
				...
				b.opts.iframe && (d += b.$iframe.offset().top - a(b.o_win).scrollTop()), ia.css("top", d)
			}
		}

		function Z() {
			var c = ia.data("origin"),
				d = ia.data("release-position");
			if (c !== d) {
				var e = ia.data("first"),
					f = ia.data("second"),
					g = ia.data("table"),
			...
			ia.removeData("origin"), ia.removeData("release-position"), ia.removeData("first"), ia.removeData("second"), ia.removeData("table"), b.undo.saveStep()
		}
		...
		function aa(a) {
			if (ga().length > 1 && la && F(), la === !1 && ka === !1 && oa === !1) na && clearTimeout(na), na = setTimeout(X, 30, a);
			else if (oa) {
				var c = a.pageX - b.win.pageXOffset;
				b.opts.iframe && (c += b.$iframe.offset().left);
				var d = ia.data("max-left"),
					e = ia.data("max-right");
				c >= d && e >= c ? ia.css("left", c - b.opts.tableResizerOffset) : d > c && parseFloat(ia.css("left"), 10) > d - b.opts.tableResizerOffset ? ia.css("left", d - b.opts.tableResizerOffset) : c > e && parseFloat(ia.css("left"), 10) < e - b.opts.tableResizerOffset && ia.css("left", e - b.opts.tableResizerOffset)
			} else la && S()
		}
		...
		function da() {
			b.shared.$ti_helper || (b.shared.$ti_helper = a('<div class="fr-insert-helper"><a class="fr-floating-btn" role="button" tabindex="-1" title="' + b.language.translate("Insert") + '"><svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg"><path d="M22,16.75 L16.75,16.75 L16.75,22 L15.25,22.000 L15.25,16.75 L10,16.75 L10,15.25 L15.25,15.25 L15.25,10 L16.75,10 L16.75,15.25 L22,15.25 L22,16.75 Z"/></svg></a></div>'), b.events.bindClick(b.shared.$ti_helper, "a", function() {
				var a = ja.data("selected-cell"),
					c = ja.data("position"),
					d = ja.data("instance") || b;
				...
				S()
			}, !0)), ja = b.shared.$ti_helper, b.tooltip.bind(b.$box, ".fr-insert-helper > a.fr-floating-btn")
		}

		function ea() {
			if (!b.$wp) return !1;
			if (!b.helpers.isMobile()) {
				la = !1, ka = !1, oa = !1, b.events.$on(b.$el, "mousedown", M), b.popups.onShow("image.edit", function() {
					E(), la = !1, ka = !1
					...
					var d = b.$el.get(0).querySelectorAll(".fr-selected-cell");
					if (d.length > 0) {
						if (c.which == a.FE.KEYCODE.ESC && b.popups.isVisible("table.edit")) return E(), b.popups.hide("table.edit"), c.preventDefault(), c.stopPropagation(), c.stopImmediatePropagation(), !1;
						if (d.length > 1) return c.preventDefault(), !1
					}
				}, !0);
				var c;
